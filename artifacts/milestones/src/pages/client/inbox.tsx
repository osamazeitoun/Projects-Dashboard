...
export default function ClientInbox() {
  const { data, isLoading, isError } = useListClientReviews();
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      <header>
        ...
        <p className="text-muted-foreground mt-1">
          Approve or reject each proposed date change. Your decision is recorded
      ...
      {data.length === 0 ? (
        <Card className="p-12 text-center text-muted-foreground border-dashed">
          You're all caught up — no date changes are waiting for your decision.
        </Card>
      ) : (
        <div className="space-y-3">
          {data.map((ev) => {
            const shift = differenceInDays(
              new Date(ev.proposedNewDate),
              new Date(ev.oldDate),
            );
            return (
                    ...
                    <div className="flex-1 min-w-0 space-y-2">
                      ...
                      <div className="flex items-center gap-3 text-sm">
                        <span className="line-through text-muted-foreground">
                          {format(new Date(ev.oldDate), "MMM d, yyyy")}
                        </span>
