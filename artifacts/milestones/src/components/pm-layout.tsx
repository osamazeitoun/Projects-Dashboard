...
import { useGetPmProjectSummary, useListProjects } from "@workspace/api-client-react";
import {
  Briefcase,
  LayoutDashboard,
  CalendarRange,
...
export default function PmLayout({ children }: { children: ReactNode }) {
  ...
  const navItems = [
    { name: "Dashboard", href: "/pm", icon: LayoutDashboard, match: (l: string) => l === "/pm" },
    { name: "Master Schedule", href: "/pm/schedule", icon: CalendarRange, match: (l: string) => l.startsWith("/pm/schedule") || l.startsWith("/pm/milestone") },
    { name: "Companies", href: "/pm/companies", icon: Building2, match: (l: string) => l.startsWith("/pm/companies") },
    {
      name: "Change Events",
      href: "/pm/change-events",
      icon: GitPullRequestArrow,
      match: (l: string) => l.startsWith("/pm/change-events") || l.startsWith("/pm/change-event"),
          ...
          {summary ? (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="w-full text-left bg-muted/60 hover:bg-muted rounded-md px-3 py-2 flex items-start justify-between gap-2 transition-colors">
                  <div className="min-w-0">
                    <div className="text-[10px] uppercase tracking-wide text-muted-foreground font-medium">{summary.projectCode}</div>
                    <div className="text-sm font-bold leading-tight truncate">{summary.projectName}</div>
                  </div>
              ...
              <DropdownMenuContent className="w-64">
                {(projects ?? []).map((p) => (
                  <DropdownMenuItem key={p.id} onClick={() => setProjectId(p.id)} className="flex flex-col items-start gap-0.5">
                    <span className="text-[10px] text-muted-foreground font-mono">{p.code}</span>
                    <span className="font-medium">{p.name}</span>
                    <span className="text-xs text-muted-foreground">{p.milestoneCount} milestones · {p.companyCount} companies</span>
                  </DropdownMenuItem>
                ))}
                {otherProjects.length === 0 && (
                  <div className="px-2 py-1.5 text-xs text-muted-foreground">No other projects</div>
                )}
              </DropdownMenuContent>
            </DropdownMenu>
          ) : (
            ...
            return (
              ...
              >
                ...
                {item.count !== undefined && item.count > 0 && (
                  <span className={`text-xs px-2 py-0.5 rounded-sm font-medium font-mono ${item.countColor ?? "bg-ink text-[color:var(--c-accent-fg)]"}`}>
                    {item.count}
                  </span>
                )}
              </Link>
